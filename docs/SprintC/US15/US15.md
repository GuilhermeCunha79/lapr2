...
-Typed Data: Results date; Chemical analysis date; Diagnosis date.
...
| Step 2                 |							 |             |                              |
| Step 3                           |	...saving the typed data? | TestValidation  | **IE:** TestValidation knows its own data 						 |             |                              |
| 		                 |	... instantiating a new validation? | TestStore   | **Creator (R1)** and **HC+LC**: By the application of the Creator (R1) it would be the "Company". But, by applying HC + LC to the "Company", this delegates that responsibility to the "ClientStore"   |
|  		 			     |  ... knows ValidationStore?	 |  Company   |  **IE:** Company knows the ValidationStore to which it is delegating some tasks |
|  		                 |	... validating all data (local validation)? | TestValidation | **IE:** an object knows its data|
| 			  		     |	... validating all data (global validation)? | TestStore | **IE:** knows all the validations|
| Step 4                 |	                |                   |                   |	
