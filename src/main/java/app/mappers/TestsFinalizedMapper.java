...
public class TestsFinalizedMapper {
    ...
    public static List<String> toDtoFin(List<ClinicalTest> readyToValidate) {
        ...
        for (ClinicalTest test : readyToValidate) {
            rtvListDto.add(String.format("%s Collected at: %s | Reported at: %s | Validated at: %s | Report date: %s | %n| Report: %s |",test.toString(), test.getChemicalAnalysisDate(), test.getReportDate(), test.getValidationDate(), test.getReportDate(), test.getReportText()));
        }
        return rtvListDto;
    }
}
